...
def prompt_module(config):
    return import_module(f"memmachine.server.prompt.{config["prompt_module"]}", __package__)
...
class TestLongMemEvalIngestion:
    ...
    async def eval_answer(
        ...
        profile_search_resp = await profile_memory.semantic_search(question_str,
                                                                   user_id=user_id)
    ...
    async def test_periodic_mem_eval(self, long_mem_convos, long_mem_question, long_mem_answer, profile_memory,
                                     llm_model):
        ...
        assert "The relevant information is not found in the user's persona profile" not in eval_resp
